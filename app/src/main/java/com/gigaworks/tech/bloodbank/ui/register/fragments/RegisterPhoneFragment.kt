...
class RegisterPhoneFragment : BaseFragment<FragmentRegisterPhoneBinding>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.loginBtn.setOnClickListener {
            if (isValidForm()) {
                ...
                hideKeyboard()
                val bundle = bundleOf(
                    ConfirmRegisterFragment.PHONE_NUMBER to binding.phoneNumber.text.toString()
                        .trim()
                )
                findNavController().navigate(
                    R.id.action_registerPhoneFragment_to_confirmRegisterFragment,
                    bundle
                )
            }
        }
    ...
    override fun getViewBinding(
        inflater: LayoutInflater,
        container: ViewGroup?
    ) = FragmentRegisterPhoneBinding.inflate(inflater, container, false)
}